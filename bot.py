...
MASTER_INFO = {
    "Артем": {
        ...
        "description": "👤 Артем\n💈 Барбер\n⭐️ Опыт: 5 років\n✂️ Специализация: стрижки, fade, борода"
    },
    "Даня": {
        "photo": "https://i.postimg.cc/1Rfd5cRk/barber-dan.jpg",
        "description": "👤 Даня\n💈 Барбер\n⭐️ Опыт: 3 роки\n✂️ Специализация: чоловічі стрижки, борода"
    },
    "Максим": {
        "photo": "https://i.postimg.cc/YSP9czCC/max.jpg",
        "description": "👤 Максим\n💈 Барбер\n⭐️ Опыт: 4 роки\n✂️ Специализация: классика, fade"
    },
}

DB_FILE = "barbershop_ru.db"
...
def admin_kb():
    return kb([
        ["📋 Список записьей", "📊 Аналитика"],
        ["🕒 Занятые слоты"],
        ["❌ Удалить записьь", "🧹 Очистить всё"],
        ["⚙️ Настройки"],
        ["🚪 Выйти из админки"],
    ])
...
async def get_master(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if master not in master_names:
        await update.message.reply_text("Выбери мастера кнопкою.")
        return MASTER
...
async def get_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
    "💬 Добавь комментарий к записьи или нажми 'Пропустить':",
    reply_markup=kb([
        ["⏭ Пропустить"],
        ["⬅️ Назад"]
    ])
...
async def reminder_checker(app):
    while True:
        ...
        for record in records:
            ...
            # Напоминание только примерно за 1 час до записьи
...
async def show_my_records(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not records:
        await update.message.reply_text(
            "У тебя пока нет записьей.",
            reply_markup=main_menu_kb()
        )
        return

    message = "📋 Твои записьи:\n\n"
    keyboard = []
...
async def send_message_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if records:
        ...
        status = "⚪ Клиент пока не записьан"
        appointments_text = "\nУ клиента нет записьей"
...
async def admin_cancel_comment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=user_id,
            text=(
                f"❌ Ваша записьь отменена администратором.\n\n"
                f"💈 Услуга: {service}\n"
                f"👤 Мастер: {master}\n"
                f"📅 Дата: {date}\n"
                f"🕒 Время: {time}\n\n"
    ...
    await update.message.reply_text(
        "✅ Записьь отменена, клиенту отправлено сообщение.",
        reply_markup=admin_kb()
    )
...
async def cancel_user_record(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Ошибка отмены записьи.")
        return

    record = get_record(record_id)

    if not record or record[1] != user_id:
        await update.message.reply_text("Эта записьь не найдена.")
        return
...
async def admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📋 Список записьей":
        if not records:
            await update.message.reply_text("Записьей нет.", reply_markup=admin_kb())
            return
    ...
    elif text == "❌ Удалить записьь":
        if not records:
            await update.message.reply_text("Записьей нет.", reply_markup=admin_kb())
            return

        keyboard = [[f"❌ Запись {r[0]}"] for r in records]
        keyboard.append(["⬅️ Назад"])

        await update.message.reply_text(
            "Выбери записьь для отмены:",
            reply_markup=kb(keyboard)
        )

    elif text.startswith("❌ Запись "):
        try:
            record_id = int(text.split()[-1])
        except ValueError:
        ...
        if not record:
            await update.message.reply_text("Записьь не найдена.", reply_markup=admin_kb())
            return
        ...
        message += "━━━━━━━━━━━━━━\n"
        message += "📌 Всего\n"
        message += f"• Активных записьей: {len(normalized_records)}\n"
        message += f"• Уникальных клиентов: {len(set(r['client_id'] for r in normalized_records))}\n"
        message += f"• Ближайшая записьь: {nearest_text}\n\n"

        message += "━━━━━━━━━━━━━━\n"
        message += f"📅 Сегодня — {today_str}\n"
        message += f"• Записьей: {len(today_records)}\n\n"
        ...
        if today_records:
            ...
            message += "На сегодня записьей нет\n\n"
        ...
        for i, (master, count) in enumerate(sorted_masters):
            if i == 0:
                message += f"🏆 {master} — {count} записьей\n"
            else:
                message += f"• {master} — {count} записьи\n"
        ...
        clear_records()
        await update.message.reply_text("🧹 Все записьи удалены.", reply_markup=admin_kb())
        ...
        await update.message.reply_text(
            ...
            "Например:\n"
            "Детская стрижка | 400 грн | 30 мин",
            reply_markup=back_kb([])
        )
        ...
        await update.message.reply_text("✅ Мастер удалён.", reply_markup=admin_kb())
...
def main():
    ...
    booking = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^📝 Записьаться$"), begin_booking)
        ],
        states={
    ...
    app.add_handler(MessageHandler(filters.Regex("^📋 Мои записьи$"), show_my_records))
    app.add_handler(
        MessageHandler(
